...
| `HF_ORG` | 선택 | `Himedia-AI-01` (기본) | Hub 업로드 대상 org. 빈 문자열(`HF_ORG=""`)로 두면 토큰 소유자의 개인 계정에 업로드 |
...
| `DATA_DIR` | `data` | 데이터셋 경로 (기본은 repo 루트의 `data/`). 다른 위치면 절대 경로 지정 |
...
- `data/` (또는 `$DATA_DIR`) 데이터셋 존재 여부 확인
...
| Max steps | -1 (epoch 사용) | `MAX_STEPS=250` (epoch 무시하고 step 수 직접 제어) |
